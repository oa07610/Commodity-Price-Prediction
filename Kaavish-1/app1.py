...
SUGAR_HEAT = 'data/SUGAR_dummy.csv'
#MAIZE_HEAT = 
#COTTON_HEAT =
#WHEAT_HEAT=
SUGAR_PRED = 'data/SUGAR_dummy.csv'
#MAIZE_PRED = 
#COTTON_PRED =
#WHEAT_PRED=
# Mapping of crops to their respective CSV filenames
CROP_FILES = {
    'Sugar': {'actual': SUGAR_HEAT, 'pred': SUGAR_PRED},
    #'Maize': {'actual': MAIZE_HEAT, 'pred': MAIZE_PRED},
    #'Cotton': {'actual': COTTON_HEAT, 'pred': COTTON_PRED},
    #'Wheat': {'actual': WHEAT_HEAT, 'pred': WHEAT_PRED}
}
...
def get_chart_data():
    """
    Endpoint to fetch data for the line chart based on selected crop and regions.
    Expected query parameters:
    - crop: Name of the crop (e.g., Cotton, Maize, Sugar, Wheat)
    - regions: Comma-separated list of regions (e.g., Punjab,Sindh)
    - by_product: Optional parameter for additional filtering
    """
    crop = request.args.get('crop')
    regions = request.args.get('regions')  # Comma-separated string
    by_product = request.args.get('by_product', None)  # Optional

    if not crop or crop not in CROP_FILES:
        return jsonify({'error': 'Invalid or missing crop parameter.'}), 400

    regions_list = [region.strip() for region in regions.split(',')] if regions else []

    actual_file = CROP_FILES[crop]['actual']
    pred_file = CROP_FILES[crop]['pred']
    ...
    try:
        ...
        return jsonify({'error': f'Data file for {crop} not found.'}), 404
    except Exception as e:
        return jsonify({'error': f'Error reading data files: {str(e)}'}), 500

    # Filter by by_product if specified
    if by_product:
        try:
            by_product = int(by_product)
            actual_data = actual_data[actual_data['by_product'] == by_product]
            pred_data = pred_data[pred_data['by_product'] == by_product]
        except ValueError:
            return jsonify({'error': 'Invalid by_product parameter.'}), 400

    # Filter by regions
    if regions_list:
        actual_data = actual_data[actual_data['province'].isin(regions_list)]
        pred_data = pred_data[pred_data['province'].isin(regions_list)]
    ...
    for i, region in enumerate(regions_list):
        ...
        dates_actual = region_actual['date'].dropna().tolist()
        dates_predicted = region_pred['date'].dropna().tolist()

        actual_prices = region_actual['price'].dropna().tolist()
        predicted_prices = region_pred['price'].dropna().tolist()
...
def get_heatmap_data():
    """
    Endpoint to fetch data for the heatmap based on selected crop.
    ...
    if df.empty:
        return jsonify({'error': 'No data available for the selected crop.'}), 404
    ...
    heatmap_data = latest_df[['region', 'latitude', 'longitude', 'price']].to_dict(orient='records')
